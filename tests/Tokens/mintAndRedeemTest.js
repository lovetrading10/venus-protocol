...
describe('VToken', function () {
  ...
  describe('mintFresh', () => {
    ...
    it("fails if not fresh", async () => {
      ...
      expect(await mintFresh(vToken, minter, mintAmount)).toHaveTokenFailure('MARKET_NOT_FRESH', 'MINT_FRESHNESS_CHECK');
    });
    ...
    it("fails if insufficient approval", async () => {
      expect(
        await send(vToken.underlying, 'approve', [vToken._address, 1], {from: minter})
      ).toSucceed();
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if not fresh", async () => {
        await fastForward(vToken);
        expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('MARKET_NOT_FRESH', 'REDEEM_FRESHNESS_CHECK');
      });
      ...
      it("fails if insufficient protocol cash to transfer out", async() => {
        ...
        expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'REDEEM_TRANSFER_OUT_NOT_POSSIBLE');
      });

      it("fails if exchange calculation fails", async () => {
        if (redeemFresh == redeemFreshTokens) {
          expect(await send(vToken, 'harnessSetExchangeRate', ['0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF'])).toSucceed();
          expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('MATH_ERROR', 'REDEEM_EXCHANGE_TOKENS_CALCULATION_FAILED');
        } else {
          expect(await send(vToken, 'harnessSetExchangeRate', [0])).toSucceed();
          expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('MATH_ERROR', 'REDEEM_EXCHANGE_AMOUNT_CALCULATION_FAILED');
        }
      });

      it("fails if transferring out fails", async () => {
        await send(vToken.underlying, 'harnessSetFailTransferToAddress', [redeemer, true]);
        await expect(redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).rejects.toRevert("revert TOKEN_TRANSFER_OUT_FAILED");
      });

      it("fails if total supply < redemption amount", async () => {
        await send(vToken, 'harnessExchangeRateDetails', [0, 0, 0]);
        expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('MATH_ERROR', 'REDEEM_NEW_TOTAL_SUPPLY_CALCULATION_FAILED');
      });

      it("reverts if new account balance underflows", async () => {
        await send(vToken, 'harnessSetBalance', [redeemer, 0]);
        expect(await redeemFresh(vToken, redeemer, redeemTokens, redeemAmount)).toHaveTokenFailure('MATH_ERROR', 'REDEEM_NEW_ACCOUNT_BALANCE_CALCULATION_FAILED');
      });
  ...
  describe('redeem', () => {
    ...
    it("returns error from redeemFresh without emitting any extra logs", async () => {
      ...
      expect(await quickRedeem(vToken, redeemer, redeemTokens, {exchangeRate})).toHaveTokenFailure('TOKEN_INSUFFICIENT_CASH', 'REDEEM_TRANSFER_OUT_NOT_POSSIBLE');
    });
